...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our VCRo Hub account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'our VCRo Hub account';
    const message = `Join ${accountName} on VCRo Hub using this link (valid for ${result?.expiresInDays} days): ${url}`;
